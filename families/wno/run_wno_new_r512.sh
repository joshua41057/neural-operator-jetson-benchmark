...
RUN_TAG="${RUN_TAG:-wno_exact_new_r512_$(date +%Y%m%d_%H%M%S)}"
...
for precision in "${precisions[@]}"; do
  for rep in $(seq 1 "${REPS}"); do
    run_one "wno_burgers_base_r512" "burgers" "checkpoints/wno_burgers_base_r512.pth" \
      "/home/jetson/data/wno_inference_banks_exact/burgers_r512_bank.pt" "${precision}" "${rep}"
  done
done
