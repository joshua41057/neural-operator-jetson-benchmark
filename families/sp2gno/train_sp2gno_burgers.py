...
def main():
    ...
    ap.add_argument('--sub', type=int, default=8, help='spatial subsample (s=8192//sub)')
    ...
    ap.add_argument('--out_dir', default='runs/burgers')
    ...
    logger = setup_logging(args.out_dir, name='burgers')
    ...
    final_test, best_val, _ = train_shared(
        ...
        eval_every=5, plot_every=50, label='burgers', logger=logger,
        ckpt_best=os.path.join(ckpt_dir, 'burgers_best.pth'),
    ...
    with open(os.path.join(args.out_dir, 'result.json'), 'w') as f:
        json.dump({'label': 'burgers', 'final_test_rel_l2': final_test,
                   'best_val_rel_l2': best_val, 'epochs': args.epochs,
                   's': s, 'N': N, 'args': vars(args)}, f, indent=2)
    logger.info(f"DONE burgers: final test rel-L2 {final_test:.4f}, best val {best_val:.4f}")
