...
def build_burgers_bank(src_path: str, resolution: int, sample_indices: list[int], out_path: Path):
    ...
    x = to_float_tensor(x_np)          # [N, L] or similar
    x = resize_1d(x, resolution)       # [N, L, 1] or equivalent based on preprocessing
    x = x[sample_indices]
...
def main():
    ...
    # reviewer-friendly deterministic held-out sample indices
    ...
    for r in [512, 1024, 2048, 4096]:
        build_burgers_bank(
            ...
            out_path=root / f"burgers_r{r}_bank.pt",
        )

    for r in [85, 141, 211, 281]:
        build_darcy_bank(
            src_path=darcy_src,
            resolution=r,
            sample_indices=sample_indices,
            out_path=root / f"darcy_r{r}_bank.pt",
        )
