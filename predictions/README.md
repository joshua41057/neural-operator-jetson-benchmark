...
| **2** | The mechanism-level cost partition persists when the same checkpoints are reinstantiated on a server-class accelerator | §8.2 | `gh200_placeholder/` — **records not yet in this repository** | **Holds with a qualification.** The two-way partition persists on both workloads; the full four-way ordering persists on one of the two. |
